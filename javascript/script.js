...
const contieneCaracteresInvalidos = (texto) => {
    const caracteresInvalidos = /[A-ZÁÉÍÓÚÑáéíóúü]/;
    return caracteresInvalidos.test(texto);
};
...
encriptar.addEventListener('click', () => {
    ...
    if (texto !== "") {
        if (contieneCaracteresInvalidos(texto)) {
            alert("El texto no debe contener letras mayúsculas ni acentuadas.");
            reset();
        } else {
...
desencriptar.addEventListener('click', () => {
	...
	if (texto != "") {
		if (contieneCaracteresInvalidos(texto)) {
			...
			remplace(desencript(textoMinusculas));
		}
	} else {
